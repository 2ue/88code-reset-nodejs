...
|---------|------|---------|---------|---------------|
| **PLUS 月卡** | 198元/月 | **50 刀** | **每天两次重置至上限** | **100 刀 + 头天结余额度** |
...
**目标**: 每天用满2次重置机会,达到150刀理论上限 (50刀头天结余 + 2次重置×50刀)
...
```env
# 首次重置时间（24小时制），不能超过19:00，至少为18:59，不然下一次重置会失败
FIRST_RESET_TIME=18:55

# 二次重置时间（24小时制，支持智能延迟重置），至少为23:59,保证在第二天之前重置一次
...
|------|---------|---------|-----------|------|
| **完全自动** | 无操作 | 18:55重置 + 23:56重置 | **100刀+头天结余额度** ✅ | 最优方案,无需干预 |
| **提前手动** | 17:00手动重置 | 18:55跳过 + 23:56重置 | **100刀+头天结余额度** ✅ | 兼容手动操作 |
| **临界手动** | 18:59手动重置 | 18:55跳过 + 23:59延迟重置 | **100刀+头天结余额度** ✅ | 智能延迟避免浪费 |
| **高余额** | 余额90%不重置 | 当日仅1次重置 | **100刀** ❌ | 损失50刀/天 |
| **过晚手动** | 19:30手动重置 | 冷却期跨天,放弃23:56 | **100刀** ⚠️ | 浪费 |
...
```
00:00-18:55：使用5刀
18:55：余额45刀，不重置（觉得"还够用"）
18:55-23:56：可以继续使用45刀
理论总额度：5 + 45 = 50 刀
```
...
```
00:00-18:55：使用5刀
18:55：余额45，重置（锁定+50 刀新额度）
18:55-23:56：可以继续使用50刀
理论总额度：5 + 50 = 55 刀
```
...
A: 编辑.env文件：
```env
FIRST_RESET_TIME=18:59   # 改为18:59，必须在19:00之前，不能设置19:00
SECOND_RESET_TIME=23:56  # 建议最晚为23:59
```
注意：
